...
numbers(4 digit-> exclusive)
    Armstrong sayi:herhangi bir 3 basamakli sayinin rakamlarinin kupleri
     toplami o sayiyi veriyorsa sayi  Armstrong sayidir
        ...
        parametre olarak sayi alsin
        sayilirin kuptoplami conteiner atamasi
        girilen conteiner
        if icinde kontrol ediniz
        % kullaniniz
        basamakdaki sayi bos bir variable olusur
    ...
    private static void armstrong(int sayi) {
        ...
        int girilenSayi=0;// sayiyi sayinin kupler toplami ile kiyaslamak icin

        while (sayi>0){
            basamaktakiSayi=sayi%10;// rakam rakam ayirdirk
            sayininKuplerToplami +=(basamaktakiSayi*basamaktakiSayi*basamaktakiSayi);
            sayi= sayi / 10;
        }
        if (girilenSayi==sayininKuplerToplami){
            System.out.println("girilenSayi armstron sayidir= " + girilenSayi);

        }else System.out.println("girilenSayi armstron sayi degildir" + girilenSayi);
    }
